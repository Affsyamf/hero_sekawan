...
from db.models import (
    Product, Purchasing, Purchasing_Detail, Stock_Movement, Stock_Movement_Detail,
    Stock_Opname, Stock_Opname_Detail, Ledger, Design, Color_Kitchen_Entry, Color_Kitchen_Entry_Detail,
    Color_Kitchen_Batch_Detail, LedgerRef
)
...
class DashboardService:
    ...
    def _get_metrics(
        ...
        stock_masuk_current = (
            self.db.query(func.sum(Purchasing_Detail.quantity))
            .join(Purchasing)
        ...
        stock_keluar_current = (
            self.db.query(func.sum(Stock_Movement_Detail.quantity))
            .join(Stock_Movement)
            .filter(Stock_Movement.date.between(start_date, end_date))
            .scalar() or 0
        )
        ...
        stock_keluar_prev = (
            self.db.query(func.sum(Stock_Movement_Detail.quantity))
            .join(Stock_Movement)
            .filter(Stock_Movement.date.between(prev_start, prev_end))
            .scalar() or 0
        )
        ...
        cost_produksi_current = (
            self.db.query(
                func.sum(Purchasing_Detail.quantity * Purchasing_Detail.price)
            )
            .join(Purchasing)
            .filter(Purchasing.date.between(start_date, end_date))
            .scalar() or 0
        ...
        selisih_opname_current = (
            self.db.query(func.sum(Stock_Opname_Detail.difference))
            .join(Stock_Opname)
            .filter(Stock_Opname.date.between(start_date, end_date))
            .scalar() or 0
        )
        ...
        selisih_opname_prev = (
            self.db.query(func.sum(Stock_Opname_Detail.difference))
            .join(Stock_Opname)
            .filter(Stock_Opname.date.between(prev_start, prev_end))
            .scalar() or 0
        )
    ...
    def _get_stock_flow_data(self, period: str, year: int) -> List[Dict]:
        ...
        for i in range(months_count - 1, -1, -1):
            ...
            stock_keluar = (
                self.db.query(func.sum(Stock_Movement_Detail.quantity))
                .join(Stock_Movement)
                .filter(Stock_Movement.date.between(month_start, month_end))
                .scalar() or 0
            )
    ...
    def _get_design_cost_data(
        ...
        designs = (
            self.db.query(
                ...
                func.count(Color_Kitchen_Entry.id).label('orders')
            )
            .join(Color_Kitchen_Entry, Design.id == Color_Kitchen_Entry.design_id)
            .filter(Color_Kitchen_Entry.date.between(start_date, end_date))
        ...
        for design in designs:
            ...
            cost_query = (
                self.db.query(
                    func.sum(
                        Color_Kitchen_Entry_Detail.quantity * 
                        func.coalesce(
                            self.db.query(func.avg(Purchasing_Detail.price))
                            .filter(Purchasing_Detail.product_id == Color_Kitchen_Entry_Detail.product_id)
                            .scalar_subquery(),
                            0
                        )
                    )
                )
                .join(Color_Kitchen_Entry, Color_Kitchen_Entry_Detail.color_kitchen_entry_id == Color_Kitchen_Entry.id)
                .join(Design, Color_Kitchen_Entry.design_id == Design.id)
                .filter(
                    Design.code == design.design,
                    Color_Kitchen_Entry.date.between(start_date, end_date)
                )
                .scalar() or 0
            )
            ...
            batch_cost_query = (
                self.db.query(
                    func.sum(
                        Color_Kitchen_Batch_Detail.quantity * 
                        func.coalesce(
                            self.db.query(func.avg(Purchasing_Detail.price))
                            .filter(Purchasing_Detail.product_id == Color_Kitchen_Batch_Detail.product_id)
                            .scalar_subquery(),
                            0
                        )
                    )
                )
                .join(Color_Kitchen_Entry, Color_Kitchen_Batch_Detail.batch_id == Color_Kitchen_Entry.batch_id)
                .join(Design, Color_Kitchen_Entry.design_id == Design.id)
                .filter(
                    Design.code == design.design,
                    Color_Kitchen_Entry.date.between(start_date, end_date)
